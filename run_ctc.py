...
if 1 in tasks:
  ...
  if args.dataset_train == 'mscoco_train':
    ...
    args.class2id_file = '../../data/mscoco_train_phone_segments_phone2id.json'
    
    with open(args.class2id_file, 'r') as f:
      class2idx = json.load(f)
    args.n_class = len(class2idx.keys())

    testset = MSCOCOSyntheticCaptionDataset(audio_root_path, audio_sequence_file, '../../data/mscoco_train_phone_segments_phone2id.json', feat_configs)
    test_loader = torch.utils.data.DataLoader(testset, batch_size=args.batch_size, shuffle=False)
  else:
