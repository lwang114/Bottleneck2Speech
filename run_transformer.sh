...
resume="exp/train_pytorch_train_li10/results/model.acc.best"        # Resume the training from snapshot
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
  ...
  local/setup_languages.sh \
    ...
    --mscoco-train "${mscoco_train}" \
    --mscoco-recog "${mscoco_recog}" \
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    echo "stage 5: Convert frame-level feature to segment-level feature"
    python convert_frame_to_segment_feature.py -e ./ -d mscoco2k -a transformer -f mean 
fi
